...
test_that("All probabilities/densities >= 0", {
  ...
  expect_true(suppressWarnings(all(ddlaplace(x, scale = 0.5) >= 0)))
  ...
  expect_true(all(dnst(x, 2, 0, 1) >= 0))
...
test_that("All cumulative probabilities >= 0 and <= 1", {
  ...
  expect_true(all(pnst(x, 2, 0, 1) >= 0 & pnst(x, 2, 0, 1) <= 1))
  ...
  expect_true(all(ptpois(x, lambda = 25, a = 0) >= 0))
